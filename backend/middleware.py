...
async def db_logging(request: Request,  call_next): 
    ...
    # Create cursor 
    cursor = conn.cursor() 
    insert_query = """INSERT INTO TABLE fastapi_logs VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""" 
    # Define data to be inserted 
    data = (0, int(response_status_code), request_method, request_url, json.dumps(request_query_params), json.dumps(request_path_parameters), timespan, created) 
